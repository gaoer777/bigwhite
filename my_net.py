    ...
    for i in range(num_self_cbam_residuals):
        if i == 0:
            blk.append(mm.Self_ObjectDetect_CBAMResidual(input_channels, num_channels, kernel_size=kernel_size,
                                                         use_1x1conv=True, strides=stride, padding=padding))
        else:
            blk.append(mm.Self_ObjectDetect_CBAMResidual(num_channels, num_channels, kernel_size=kernel_size, padding=padding))
    blk.append(mm.Cross_ObjectDetect_CBAMResidual(num_channels, num_channels, kernel_size=kernel_size, padding=padding))
    return blk


def object_detect_new_cbam_net(anchors, kernel_size, padding):
    b1 = nn.Sequential(nn.Conv2d(3, 63, kernel_size=(3, 9), padding=(1, 4), groups=3, stride=(1, 1)),
                       nn.BatchNorm2d(63), nn.ReLU())  # out-->63*512
    b2 = nn.Sequential(*object_detect_attention_block(63, 63, 2, stride=(2, 2), kernel_size=kernel_size, padding=padding))  # out-->32*256
    b3 = nn.Sequential(*object_detect_attention_block(63, 126, 2, stride=(1, 2), kernel_size=kernel_size, padding=padding))  # out-->32*128
    b4 = nn.Sequential(*object_detect_attention_block(126, 126, 2, stride=(2, 2), kernel_size=kernel_size, padding=padding))  # out-->16*64
    b5 = nn.Sequential(*object_detect_attention_block(126, 252, 2, stride=(1, 2), kernel_size=kernel_size, padding=padding))  # out-->16*32
    b6 = nn.Sequential(*object_detect_attention_block(252, 252, 2, stride=(2, 2), kernel_size=kernel_size, padding=padding))  # out-->8*16
