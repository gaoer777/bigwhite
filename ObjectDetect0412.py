...
class object_detect_attention_block(nn.Module):
    def __init__(self, input_channels, num_channels, kernel_size,
        ...
        self.CrossRes1 = my_module.Cross_ObjectDetect_CBAMResidual(input_channels, num_channels, strides,
                                                                   kernel_size=kernel_size,
                                                                   padding=padding, use_1x1conv=use_1x1conv)
        self.CrossRes2 = my_module.Cross_ObjectDetect_CBAMResidual(num_channels, num_channels, 1,
...
class ODAB(nn.Module):  # object_detect_attention_block
    def __init__(self):
        ...
        self.attention_block1 = object_detect_attention_block(16, 32, strides=(2, 2), kernel_size=(3, 3),
                                                              padding=1)  # out-->16*128
        self.attention_block2 = object_detect_attention_block(32, 64, strides=(1, 2), kernel_size=(3, 3),
                                                              padding=1)  # out-->16*64
        self.attention_block3 = object_detect_attention_block(64, 128, strides=(2, 2), kernel_size=(3, 3),
                                                              padding=1)  # out-->8*32
        self.attention_block4 = object_detect_attention_block(128, 256, strides=(1, 2), kernel_size=(3, 3),
                                                              padding=1)  # out-->8*16
        self.attention_block5 = object_detect_attention_block(256, 512, strides=(2, 2), kernel_size=(3, 3),
                                                              padding=1)  # out-->4*8
...
class Attention_module(nn.Module):
    def __init__(self, input_channels, output_channels, feature_size, strides=(1, 1),
        ...
        self.DRAM = models_c.RAM(input_channels=input_channels, output_channels=output_channels, kernel_size=(3, 3),
                                 strides=(2, 2), feature_size=feature_size, groups=groups, use_1x1conv=True,
                                 use_TA=use_TA, use_SA=use_SA)
        self.RAM1 = models_c.RAM(input_channels=output_channels, output_channels=output_channels, kernel_size=(3, 3),
                                 feature_size=feature_size, groups=groups, use_TA=use_TA, use_SA=use_SA)
        self.RAM2 = models_c.RAM(input_channels=output_channels, output_channels=output_channels, kernel_size=(3, 3),
...
class RAM_Object(nn.Module):  # object_detect_attention_block
    def __init__(self):
        super(RAM_Object, self).__init__()
        self.conv1 = nn.Conv2d(3, 16, stride=(2, 2), kernel_size=(3, 3), padding=(1, 1))  # out-->32*256
        self.attention_block1 = models_c.Attention_module(16, 32, strides=(2, 2), kernel_size=(3, 3),
                                                          padding=1)  # out-->16*128
        self.attention_block2 = object_detect_attention_block(32, 64, strides=(1, 2), kernel_size=(3, 3),
                                                              padding=1)  # out-->16*64
        self.attention_block3 = object_detect_attention_block(64, 128, strides=(2, 2), kernel_size=(3, 3),
